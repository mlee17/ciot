function [combined_high, combined_medium, combined_low, combined_average] = combinedIM%(cond, conditions_to_visualize, groups_to_visualize, target_freq, file_name)
cond = 2;
...
if length(target_freq) ~= 3
    error('the number of target frequencies does not match!!! fix the number here!!!');
end
combined_low = sqrt(mean_low(1,:).^2 + mean_low(2,:).^2 + mean_low(3,:).^2);% / 3;
combined_medium = sqrt(mean_medium(1,:).^2 + mean_medium(2,:).^2 + mean_medium(3,:).^2);% / 3;
combined_high = sqrt(mean_high(1,:).^2 + mean_high(2,:).^2 + mean_high(3,:).^2);% / 3;
combined_average = sqrt(mean_average(1,:).^2 + mean_average(2,:).^2 + mean_average(3,:).^2);% / 3;
