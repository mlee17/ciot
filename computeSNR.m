function [meanSNR, allSNR_SumFreq, lowContrastSNR] = computeSNR(file_directory,task)%(cond, conditions_to_visualize, groups_to_visualize, target_freq, file_name)
% High contrasts ONLY
cond = 2;
...
freq = [6 10 8 2]; % 2f1 2f2 f1+f2 -f1+f2 
...
% end
output = loadAxx(2, file_directory);
...
%     elseif cond == 2
        ...
        for c = conditions_to_visualize
            for iFreq = 1:length(freq)
                if freq(iFreq) == 2 && ~task
                    thisSingleConds = [8,9,17,18,26,27];
            
                    for i = 1:length(thisSingleConds)
                        cos_single(i) = thisCos(freq(iFreq)*2 + 1, thisSingleConds(i));
                        sin_single(i) = thisSin(freq(iFreq)*2 + 1, thisSingleConds(i));
                    end
                    cos_singleMean = mean(cos_single);
                    sin_singleMean = mean(sin_single);
            
                    cos_thisCond = thisCos(freq(iFreq)*2 + 1, c);
                    sin_thisCond = thisSin(freq(iFreq)*2 + 1, c);
...
%     allAmp(6,1);
%     allSidebands(6,1);
    % -f1+f2 -> Low contrast COND 19
    allAmp(4,19);
    allSidebands(4,19);
    
    thisChannelAmp = [allAmp(1,8), allAmp(2,9), allAmp(3,1), allAmp(4,1), allAmp(4,19)];
    thisChannelSide = [allSidebands(1,8), allSidebands(2,9), allSidebands(3,1), allSidebands(4,1), allSidebands(4,19)];
    
    thisChannel_SumFreqAmp = allAmp(3,:);
    thisChannel_SumFreqSide = allSidebands(3,:);
...
%         significant_average_contrasts(signal_index, :) = average_contrasts;
        ...
        occ_Amp_sum(signal_index,:) = thisChannel_SumFreqAmp;
        occ_Side_sum(signal_index,:) = thisChannel_SumFreqSide;
...
lowContrastSNR = meanSNR(end);
meanSNR = meanSNR(1:end-1);
sumSNR = occ_Amp_sum./occ_Side_sum;
allSNR_SumFreq = mean(sumSNR,1);
% 
