...
for s = 1:nSubj
    ...
    [bestfit_full, bestfit_amp, F_subj(s,1)] = compareGaussianModels(orientationAll, high_contrastsOcc(s,:), medium_contrastsOcc(s,:), low_contrastsOcc(s,:));
    
    modelComparisonPlot(orientationAll, high_contrastsOcc(s,:), medium_contrastsOcc(s,:), low_contrastsOcc(s,:), average_contrastsOcc(s,:),...
        [], [], [], [], bestfit_full, bestfit_amp, F_subj(s,1), subj_label)
...
%%%
[bestfit_full, bestfit_amp, F_obt] =compareGaussianModels(orientationAll, highAveOcc, medAveOcc, lowAveOcc);
figure; 
...
title(sprintf('Full Model Fobt = %0.4f', F_obt));
...
xlabel('Orientation (Deg)');
ylabel('F1+F2 EEG Power');
title(sprintf('Amplitude Model Fobt = %0.4f', F_obt));
...
for s = 1:nSubj
    ...
    [bestfit_full, bestfit_amp, F_subj2(s,1)] = compareGaussianModels(orientationAll, high_contrastsOcc2(s,:), medium_contrastsOcc2(s,:), low_contrastsOcc2(s,:));
    ...
    modelComparisonPlot(orientationAll, high_contrastsOcc2(s,:), medium_contrastsOcc2(s,:), low_contrastsOcc2(s,:), average_contrastsOcc2(s,:),...
        [], [], [], [], bestfit_full, bestfit_amp, F_subj2(s,1), subj_label)
end
...
[bestfit_full, bestfit_amp, F_obt2] =compareGaussianModels(orientationAll, highAveOcc2, medAveOcc2, lowAveOcc2);
figure; 
subplot(1,2,1); hold on;
...
xlabel('Orientation (Deg)');
ylabel('-F1+F2 EEG Power');
title(sprintf('Full Model Fobt = %0.4f', F_obt2));
subplot(1,2,2); hold on;
myerrorbar(orientationAll, noiseAveOcc2, 'Color', noiseColors, 'yError', noiseErrOcc2,'Symbol','o','MarkerSize',10);
plot(bestfit_amp.fitX, bestfit_amp.fitY_high, '-', 'Color', colors(7,:),'LineWidth',3); 
plot(bestfit_amp.fitX, bestfit_amp.fitY_med, '-', 'Color', colors(5,:),'LineWidth',3); 
...
xlabel('Orientation (Deg)');
ylabel('-F1+F2 EEG Power');
title(sprintf('Amplitude Model Fobt = %0.4f', F_obt2));
