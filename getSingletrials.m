...
conditions_to_analyze = [high_contrast(end-1:end); medium_contrast(end-1:end); low_contrast(end-1:end)];
signalAll = cell(3,2);
noiseLowerAll= cell(3,2);
noiseUpperAll = cell(3,2);

orientation = [-90,-70,-45,-30,-15,-7, 0, 7, 15, 30, 45, 70, 90]; % negative values included for reflection
signal_index = 1;
all_snrs = 0;

frequency_index = (target_freq / dFHz) + 1;

for iContrast = 1:3 %high med low
    for iOrientation = 1:2 % 70 90
        for iChannel = 1:length(occipital_electrodes)
            
            signalAll{iContrast,iOrientation} = [signalAll{iContrast,iOrientation};...
                squeeze(ampl(frequency_index, occipital_electrodes(iChannel),:,conditions_to_analyze(iContrast,iOrientation)))];
            noiseLowerAll{iContrast,iOrientation} = [noiseLowerAll{iContrast,iOrientation};...
