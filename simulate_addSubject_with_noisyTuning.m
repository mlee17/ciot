...
tuningFunctions =[];
for iSub = 1:100
    tuningFunctions(:,:,iSub) = [secondSumAll{iSub}{1}; secondSumAll{iSub}{2}; secondSumAll{iSub}{3}];
end
nSub = 100;%length(SNR_high0);
nDraw = 1000;
for iSelectSub = 1:nSub
    ...
    for iDraw = 1:nDraw
        ...
        for iSub = 1:iSelectSub
            thisTuningFunctions(:,:,iDraw,iSub) = tuningFunctions(:,:,thisDraw(iSub));
        end
...
for iSelectSub = 1:10
    [bestfit_full{iSelectSub}, bestfit_amp{iSelectSub}, F_obt(iSelectSub), Fr(iSelectSub), AIC_full(iSelectSub), AIC_amp(iSelectSub)] = ...
        compareGaussianModels_fixedOffset(orientationAll, meanTuningFunctions{iSelectSub}(1,:), meanTuningFunctions{iSelectSub}(2,:), meanTuningFunctions{iSelectSub}(3,:));
    ...
    plot(bestfit_full{iSelectSub}.fitX, bestfit_full{iSelectSub}.fitY_high, '-', 'Color', colors(7,:),'LineWidth',2); 
    plot(bestfit_full{iSelectSub}.fitX, bestfit_full{iSelectSub}.fitY_med, '-', 'Color', colors(5,:),'LineWidth',2);
    plot(bestfit_full{iSelectSub}.fitX, bestfit_full{iSelectSub}.fitY_low, '-', 'Color', colors(3,:),'LineWidth',2); 
    myerrorbar(orientationAll, meanTuningFunctions{iSelectSub}(1,:), 'Color', colors(7,:), 'yError', meanTuningFunctions_se{iSelectSub}(1,:),'Symbol','o','MarkerSize',6);
    myerrorbar(orientationAll, meanTuningFunctions{iSelectSub}(2,:), 'Color', colors(5,:), 'yError', meanTuningFunctions_se{iSelectSub}(2,:),'Symbol','o','MarkerSize',6);
    myerrorbar(orientationAll, meanTuningFunctions{iSelectSub}(3,:), 'Color', colors(3,:), 'yError', meanTuningFunctions_se{iSelectSub}(3,:) ,'Symbol','o','MarkerSize',6);
